...
class DummyWorkoutSummaryData {
    ...
    init() {
        ...
        // different dates for testing Daily and Weekly grouping
        let today = Date()
        let yesterday = Calendar.current.date(byAdding: .day, value: -1, to: today)!
        let twoDaysAgo = Calendar.current.date(byAdding: .day, value: -2, to: today)!
        let lastWeek = Calendar.current.date(byAdding: .day, value: -8, to: today)!
        
        // 1. Log a Preset Session: Today (Full Body)
        ...
        //preset session two dyas ago (Upper Focus)
        dummyManager.logPresetSession(
        ...
        //an exercise to test Form Accuracy & Insights (Last Week)
        let manualRecord = CompletedExerciseRecord(
            ...
            date: lastWeek,
            startTime: lastWeek,
            endTime: lastWeek.addingTimeInterval(20 * 60), // 20 mins duration
